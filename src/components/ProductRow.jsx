...
function ProductRow({ product, index }) {
  ...
  const handleDelete = () => {
    confirmDelete(
      ...
      if (result == true) {
        Promise.resolve(dispatch(deleteProduct(product.id)))
          .then(() => {
  ...
  return (
    <tr>
      ...
      <td className="p-2 border text-center border-gray-300">{product.desc}</td>
      <td className="p-2 border text-center border-gray-300">
      ...
      <td className="text-center p-2 border border-gray-300">
        <button
          className="bg-gray-200 rounded-md py-2 px-2 text-sky-700 me-2 font-semibold cursor-pointer"
          onClick={() => navigate("/edit-product/" + product.id)}
        >
          <Edit />
        </button>
        <button
          className="bg-sky-700 rounded-md py-2 px-2 text-white font-semibold cursor-pointer"
